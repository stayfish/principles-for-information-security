...
// bitset<160> SHA1(const string &path)
// {
...
// mode = 2 <=> read from string

istream &chooseStream(istream &s1, istream &s2, int mode)
{
	if (mode == MODE_FILE)
		return s1;
...
bitset<160> SHA1(const string &str, int mode)
{
	vector<bitset<512>> M;
	string s;
	bitset<1024> x;
	...
	stringstream ss(str);
	s = readstream(chooseStream(f, ss, mode));
	int l = s.length();
	int L = l;
	// x= SHA1_PAD(s);
	int n = 0;
...
string WriteHex(const bitset<160> &bit)
{
	char ch = 0;
	string str(20, 0);
	...
	for (unsigned char ch : str)
	{
		cur = int(ch);
		// 防止首位为 0，hex 不输出
		hex_str[count] = map[cur / 16];
		hex_str[count + 1] = map[cur % 16];
		count += 2;
	}
	return hex_str;
}